...
SsdMobilenetpp::SsdMobilenetpp(QObject *parent)
    : QObject{parent}, nn_post_proc_first_call(true)
{}

double SsdMobilenetpp::get_ms(timeval t)
{
    return (t.tv_sec * 1000 + t.tv_usec / 1000);
}

std::vector<int> SsdMobilenetpp::Filter_by_score(float *predictions, int rows, int cols, float confidence_thresh)
{
...
std::vector<float> SsdMobilenetpp::BB_decoding(std::vector<float> encoded_bbox, std::vector<float> anchors)
{
...
float SsdMobilenetpp::IoU(const ObjDetect_Location &a, const ObjDetect_Location &b)
{
...
std::vector<ObjDetect_Results> SsdMobilenetpp::non_max_suppression(const std::vector<float> &bb_predictions, std::vector<int> &class_index, std::vector<float> &filtered_scores, float iou_threshold)
{
...
void SsdMobilenetpp::recover_score_info(const std::vector<float> &scores, int number_of_boxes, int number_of_classes, std::vector<float> &highest_scores, std::vector<int> &class_indices)
{
...
void SsdMobilenetpp::nn_post_proc(std::unique_ptr<stai_mpu_network> &nn_model, std::vector<stai_mpu_tensor> output_infos, Frame_Results *results, float confidenceThresh, float iou_threshold, std::string model_type)
{
...
int SsdMobilenetpp::ReadLabelsFile(const std::string &file_name, std::vector<std::string> *result, size_t *found_label_count)
{
