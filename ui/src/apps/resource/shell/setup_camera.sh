...
if [ "$DCMIPP_SENSOR" != "NOTFOUND" ]; then
	...
	if [ $dual_pipe -eq 1 ]; then
		...
		echo "DCMIPP_SENSOR="$DCMIPP_SENSOR
		echo "AUX_POSTPROC="$aux_postproc
		ln -sf /dev/$V4L_DEVICE /dev/video-camera0
		ln -sf /dev/$V4L_DEVICE_PREV /dev/video-camera1
		echo "link /dev/$V4L_DEVICE_PREV to /dev/video-camera0"
		echo "link /dev/$V4L_DEVICE to /dev/video-camera1"
	else
		#Use main pipe for debayering, scaling and color conversion
		echo "Mediacontroller graph:"
		cmd "  media-ctl -d $mediadev --set-v4l2 \"'$sensorsubdev':0[fmt:$sensorbuscode/${SENSORWIDTH}x${SENSORHEIGHT}]\""
	...
	echo "V4L2_CAPS_PREV="$V4L2_CAPS
        ...
        v4l2-ctl -d /dev/v4l-subdev3 -c gamma_correction=0
        v4l2-ctl -d /dev/v4l-subdev3 -c gamma_correction=1
        v4l2-ctl -d /dev/v4l-subdev4 -c gamma_correction=0
        v4l2-ctl -d /dev/v4l-subdev4 -c gamma_correction=1
