...
void AudioSpectrumAnalyzer::enableSpecialEffects(bool enable)
{
    if (enable)
        QObject::connect(audioProbe, SIGNAL(audioBufferProbed(QAudioBuffer)),this, SIGNAL(signalGetMediaData(QAudioBuffer)));
    else
        QObject::disconnect(audioProbe, SIGNAL(audioBufferProbed(QAudioBuffer)),this, SIGNAL(signalGetMediaData(QAudioBuffer)));
}
